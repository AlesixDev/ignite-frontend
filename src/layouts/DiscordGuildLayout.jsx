      ...
      {sortedChannels.map((channel) => (
        <ContextMenu key={channel.id}>
          <ContextMenuTrigger>
            <Link
              key={channel.id}
              to={`/channels_discord/${channel.guild_id}/${channel.id}`}
              className={`${!expanded && channel.id != activeChannelId ? 'hidden' : ''}`}
            >
              <div
                className={`group relative mx-2 my-0.5 flex cursor-pointer items-center rounded px-2 py-1 pr-16 ${channel.id == activeChannelId
