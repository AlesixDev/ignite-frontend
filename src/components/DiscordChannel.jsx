...
const ChannelMessage = ({ message, prevMessage, pending }) => {
  ...
  const shouldStack = useMemo(() => {
    if (prevMessage) {
      ...
      const sentWithinMinute = (new Date(message.timestamp) - new Date(prevMessage.timestamp)) / 1000 < 60;
                ...
                <div className="relative mb-1 flex justify-start leading-none">
                  <PopoverTrigger>
                    <span className="font-semibold leading-none text-gray-100">
                      {message?.author.username} {message?.author.is_webhook && <Badge>Webhook</Badge>} {message?.author.is_bot && <Badge>Bot</Badge>}
                    </span>
                  </PopoverTrigger>
...
const DiscordChannel = ({ channel }) => {
  ...
  const fetchMessages = useCallback(async (opts = {}) => {
    console.log('Fetching messages from Discord API', channel);

    if (!channel?.id) return;

    console.log('Using Discord API to fetch messages');
    ...
    // https://discord.com/api/v9/channels/{channel.id}/messages?limit=50
    const { data } = await axios.get(`https://discord.com/api/v9/channels/${channel.id}/messages`, {
      headers: {
