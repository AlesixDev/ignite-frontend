        ...
        <>
            {permissionsList.map((permission, index) => (
                <div key={index}>
                    ...
                    <input type="radio" name={permission.name} checked={getPermissionState(index) == 'revoke'} onChange={() => onPermissionChange(index, 'revoke')} /> Revoke
                    <input type="radio" name={permission.name} checked={getPermissionState(index) == null} onChange={() => onPermissionChange(index, null)} /> Inherit
                    <input type="radio" name={permission.name} checked={getPermissionState(index) == 'grant'} onChange={() => onPermissionChange(index, 'grant')} /> Grant
                </div>
            ))}
        </>
    )
...
const ChannelDialog = ({ isOpen, setIsOpen }) => {
    ...
    const savePermissions = () => {
        ...
        for (const role in permissions) {
            ...
            api.put(`/channels/${selectedRole}/permissions/${selectedRole}`, {
        ...
        <div className="modal">
            <div className="modal-content">
                <div className="tabs">
                    ...
                    <button onClick={() => setActiveTab('delete')} className={activeTab === 'delete' ? 'active' : ''}>Delete Channel</button>
                </div>
                    ...
                    {activeTab === 'permissions' && (
                        <div className="permissions-container">
                            <div className="role-tabs">
                                {rolesList.map((role) => (
                                    <button key={role.id} onClick={() => setSelectedRole(role.id)} className={selectedRole === role.id ? 'active' : ''}>{role.name}</button>
                                ))}
                            </div>
                            <div className="role-permissions">
                                ...
                                <PermissionsList permissions={permissions[selectedRole]} onPermissionChange={handlePermissionChange} />
                            </div>
